...
function statusLabel(status: string) {
  ...
  if (status === "cancelled" || status === "revoked") return "Annulé";
...
export default function DailySessionFollowupSummary({ sessionId }: { sessionId: string }) {
  ...
  const signatures = summary.signatures ?? { total: 0, pending: 0, viewed: 0, signed: 0, expired: 0, failed: 0, items: [] };
  return <section style={{ padding: "1rem", background: "#fffaf0", border: "1px solid #d8b989", marginBottom: "1rem" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: ".65rem" }}>
      ...
      <Metric label="Signatures" value={`${signatures.signed}/${signatures.total}`} detail={`${signatures.pending} en attente · ${signatures.expired} expirée(s) · ${signatures.failed} échec(s)`} />
    ...
    <div style={{ marginTop: "1rem", borderTop: "1px solid #d8b989", paddingTop: "1rem" }}>
      ...
      <p style={{ marginTop: 0, color: "#70503b", lineHeight: 1.5 }}>Le statut « signé » provient uniquement de la signature enregistrée. Une consultation du lien reste affichée comme « consulté · signature attendue ». La date d’envoi n’est pas déduite de la création du lien : elle reste non tracée tant qu’aucune preuve d’email n’est reliée.</p>
      {signatures.items.length === 0 ? <p style={{ color: "#70503b" }}>Aucun document avec signature n’est enregistré pour cette session.</p> : <div style={{ display: "grid", gap: ".6rem" }}>
        {signatures.items.map((item) => <article key={item.id} style={{ padding: ".8rem", border: "1px solid #d8b989", background: "#fff" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: ".75rem", flexWrap: "wrap" }}>
            ...
            <span style={{ fontWeight: 800 }}>{statusLabel(item.status)}</span>
          </div>
          ...
          {item.last_error ? <p style={{ marginBottom: 0, color: "#7b2f21" }}>Erreur : {item.last_error}</p> : null}
        </article>)}
      </div>}
    </div>
  </section>;
