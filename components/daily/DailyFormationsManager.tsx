...
function registrationUrl(token?: string | null) { return token ? `https://www.selen-editions.fr/i/${token}` : ""; }
function qrUrl(token?: string | null, download = false) { return token ? `/api/public-registration-qr/${encodeURIComponent(token)}${download ? "?download=1" : ""}` : ""; }

export default function DailyFormationsManager({ creationMode }: { creationMode?: "program_import" | "selen_form" }) {
  ...
  const [formOpen, setFormOpen] = useState(Boolean(creationMode));
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const response = await assistanceFetch("/api/client/daily/formations", { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, creation_mode: creationMode ?? "selen_form", id: editingId, status: editingId ? form.status : "draft" }) });
  ...
  return <main style={s.main}>
    ...
    <section style={s.accordion}>
      ...
      {formOpen ? <form onSubmit={save} style={s.formPanel}>
        ...
        {creationMode === "program_import" && !editingId ? <InfoBox><b>Programme importé :</b> commencez par déposer le programme original. Les champs descriptifs déjà présents dans ce document ne doivent pas être ressaisis inutilement.</InfoBox> : null}
    ...
    <section style={s.listSection}>
      <div><p style={s.eyebrow}>Catalogue</p><h2 style={s.h2}>Formations déjà saisies</h2></div>
      {visibleFormations.length === 0 ? <div style={s.empty}>Aucune formation enregistrée pour le moment.</div> : <div style={s.cards}>{visibleFormations.map((formation) => {
        const canShare = formation.status === "validated" && formation.public_registration_enabled && formation.public_registration_token;
        const publicUrl = registrationUrl(formation.public_registration_token);
        return <article key={formation.id} style={s.formationCard}>
          <div style={s.cardHead}><div><span style={s.badge}>{statusLabel(formation.status)}</span><h3 style={s.h3}>{formation.title}</h3><p style={s.muted}>{formation.duration_hours} h · {formation.duration_days} j · {formation.modality}</p></div><strong style={s.version}>v{formation.version}</strong></div>
          <p style={s.objective}>{formation.global_objective}</p>
          {canShare ? <div style={s.registrationPanel}>
            <div style={s.registrationInfo}>
              <span style={s.registrationEyebrow}>Dossier d'inscription public</span>
              <strong>Votre lien est prêt à être diffusé</strong>
              <code style={s.registrationCode}>{publicUrl}</code>
              <p style={s.registrationHelp}>Utilisez ce lien ou ce QR code sur votre site internet, dans vos emails ou sur vos supports de communication.</p>
              <div style={s.registrationActions}>
                <button type="button" style={s.secondary} onClick={() => void copyRegistrationLink(formation.public_registration_token)}>Copier le lien</button>
                <a href={publicUrl} target="_blank" rel="noreferrer" style={s.secondaryLink}>Tester le lien</a>
                <a href={qrUrl(formation.public_registration_token, true)} style={s.secondaryLink}>Télécharger le QR code</a>
              </div>
            </div>
          ...
          <div style={s.actions}><button type="button" style={s.secondary} onClick={() => editFormation(formation)}>Modifier</button><button type="button" style={s.secondary} onClick={() => void action("duplicate", formation.id)}>Dupliquer</button><button type="button" style={s.danger} onClick={() => void action("archive", formation.id)}>Archiver</button></div>
        </article>;
      })}</div>}
    </section>
  </main>;
...
const s: Record<string, React.CSSProperties> = {
  ...
  questionCard: { border: "1px solid #e0c99d", borderRadius: 14, background: "white", padding: "1rem" }, questionHead: { display: "flex", justifyContent: "space-between", marginBottom: 10 }, primary: { border: 0, borderRadius: 10, background: "#74401f", color: "white", padding: ".72rem 1rem", fontWeight: 800, cursor: "pointer" }, secondary: { border: "1px solid #c9ad7d", borderRadius: 10, background: "#fffaf0", color: "#5d3b22", padding: ".65rem .9rem", fontWeight: 700, cursor: "pointer" }, secondaryLink: { display: "inline-flex", alignItems: "center", border: "1px solid #c9ad7d", borderRadius: 10, background: "#fffaf0", color: "#5d3b22", padding: ".65rem .9rem", fontWeight: 700, textDecoration: "none" }, smallButton: { border: "1px solid #d8b989", borderRadius: 8, background: "white", color: "#6a4528", padding: ".45rem .65rem", cursor: "pointer" }, danger: { border: "1px solid #c79688", borderRadius: 10, background: "#fff6f2", color: "#934d3a", padding: ".65rem .9rem", fontWeight: 700, cursor: "pointer" },
  formActions: { display: "flex", gap: 10, flexWrap: "wrap" }, actions: { display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }, checkboxGrid: { display: "flex", gap: 10, flexWrap: "wrap" }, check: { display: "flex", gap: 6, alignItems: "center", padding: ".55rem .7rem", border: "1px solid #dec79e", borderRadius: 10 }, money: { display: "flex", gap: 8, alignItems: "center" }, link: { display: "inline-block", marginTop: 7, color: "#74401f", fontWeight: 700 },
  listSection: { display: "grid", gap: 14 }, cards: { display: "grid", gap: 12 }, formationCard: { padding: "1.2rem", border: "1px solid #d8b989", background: "#fffaf0", borderRadius: 16 }, cardHead: { display: "flex", justifyContent: "space-between", gap: 15 }, badge: { display: "inline-block", fontSize: 11, fontWeight: 800, padding: ".3rem .55rem", borderRadius: 999, background: "#f0e0bd", color: "#70401f" }, version: { fontSize: 12, color: "#8a4b24" }, objective: { margin: ".7rem 0", lineHeight: 1.55 }, empty: { padding: "1.5rem", border: "1px dashed #d8b989", borderRadius: 14, color: "#806a58" },
  registrationPanel: { marginTop: 14, padding: 14, borderRadius: 14, border: "1px solid #d7c29b", background: "#fff", display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 16, alignItems: "center" }, registrationInfo: { display: "grid", gap: 7 }, registrationEyebrow: { fontSize: 11, fontWeight: 800, color: "#8a4b24", textTransform: "uppercase", letterSpacing: ".08em" }, registrationCode: { display: "block", maxWidth: "100%", overflowWrap: "anywhere", padding: ".6rem .7rem", borderRadius: 8, background: "#f7f0e5", color: "#5d3b22" }, registrationHelp: { margin: 0, color: "#806a58", lineHeight: 1.45 }, registrationActions: { display: "flex", gap: 8, flexWrap: "wrap" }, qrWrap: { display: "grid", justifyItems: "center", gap: 5, fontSize: 11, color: "#806a58" },
};
