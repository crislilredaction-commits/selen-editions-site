...
type Formation = {
  ...
  detailed_program?: string | null; detailed_program_document_url?: string | null; positioning_questionnaire_document_url?: string | null; accessibility: string; disability_referent?: string | null;
    ...
    <section style={s.accordion}>
      ...
      {formOpen ? <form onSubmit={save} style={s.formPanel}>
        ...
        <div style={s.formGrid}>
          ...
          <Field full label="Contenu détaillé de la formation *" help="Décrivez le déroulé complet de la formation : modules, séquences, thèmes et contenus abordés. Ce contenu figurera dans le programme officiel."><textarea required value={form.detailed_program} onChange={(e) => setForm({ ...form, detailed_program: e.target.value })} style={s.textarea} rows={10} /></Field>
...
const s: Record<string, React.CSSProperties> = {
  ...
  listSection: { display: "grid", gap: 14 }, cards: { display: "grid", gap: 12 }, formationCard: { padding: "1.2rem", border: "1px solid #d8b989", background: "#fffaf0", borderRadius: 16 }, cardHead: { display: "flex", justifyContent: "space-between", gap: 15 }, badge: { display: "inline-block", fontSize: 11, fontWeight: 800, padding: ".3rem .55rem", borderRadius: 999, background: "#f0dfbd", color: "#6d421f" }, version: { color: "#9a7448" }, objective: { lineHeight: 1.55 }, empty: { padding: "2rem", textAlign: "center", border: "1px dashed #d8b989", borderRadius: 16, color: "#806a58" },
  registrationPanel: { display: "grid", gridTemplateColumns: "minmax(0,1fr) 160px", gap: 18, alignItems: "center", marginTop: 16, padding: 16, border: "1px solid #cdb786", borderRadius: 14, background: "#fffdf8" }, registrationInfo: { display: "grid", gap: 8 }, registrationEyebrow: { fontSize: 10, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: "#8a4b24" }, registrationCode: { display: "block", padding: ".65rem .75rem", border: "1px solid #e2d0ad", borderRadius: 9, background: "#fff", overflowWrap: "anywhere", fontSize: 12 }, registrationHelp: { margin: 0, color: "#806a58", lineHeight: 1.45, fontSize: 12 }, registrationActions: { display: "flex", gap: 8, flexWrap: "wrap" }, qrWrap: { display: "grid", justifyItems: "center", gap: 6, fontSize: 10, color: "#806a58" },
};