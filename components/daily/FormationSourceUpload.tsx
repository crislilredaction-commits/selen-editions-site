...
export default function FormationSourceUpload({ kind, label, value, onUploaded, help }: Props) {
  ...
  return <div style={{ display: "grid", gap: 7 }}>
    ...
    <label style={{ display: "inline-flex", width: "fit-content", alignItems: "center", gap: 8, border: "1px solid #8a4b24", borderRadius: 10, padding: "9px 12px", background: "#fffaf0", fontWeight: 700, cursor: uploading ? "wait" : "pointer" }}>
      <input type="file" accept={PROGRAMME_ACCEPT_ATTRIBUTE} disabled={uploading} onChange={upload} style={{ position: "absolute", width: 1, height: 1, overflow: "hidden", clip: "rect(0 0 0 0)" }} />
    ...
    {error ? <span role="alert" style={{ fontSize: 12, color: "#9b3d2d" }}>{error}</span> : null}
  </div>;
}
