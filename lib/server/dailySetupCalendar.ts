...
const PREP_DOCUMENTS = [
  "Avis de situation INSEE",
  "Dernier BPF si vous en avez un",
  "CV du ou des formateurs",
  "Programme de la ou des formations à paramétrer",
  "Certificat Qualiopi si votre organisme est certifié",
];
...
export async function getNextDailySetupSlot(maxDays=60){const c=config();const now=new Date();for(let offset=0;offset<=maxDays;offset++){const candidate=new Date(now.getTime()+offset*86400000);const date=dateKey(candidate,c.timezone);if(!windowFor(date))continue;const slots=await getDailySetupSlots(date);if(slots.length)return{date,slot:slots[0]};}return null}
