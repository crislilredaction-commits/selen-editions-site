...
function organisationFooter(common: DailyPretrainingCommon, generated: string) {
  const nda = clean(common.organisationNda)
    ? `NDA ${esc(common.organisationNda)} · Cette déclaration ne vaut pas agrément de l'État`
    : "NDA non renseigné";
  return `${esc(common.organisationName || "Organisme de formation")} · ${multiline(common.organisationAddress, "Adresse non renseignée")} · SIRET ${esc(common.organisationSiret || "non renseigné")} · ${nda} · ${esc(common.organisationPhone || "Téléphone non renseigné")} · ${esc(common.organisationEmail || "Email non renseigné")} · Document actualisé le ${esc(generated)}`;
}
