...
export async function buildDailyPortalAuthEntryUrl(input: BuildDailyPortalAuthEntryInput) {
  ...
  if (user?.user_metadata?.selen_password_configured === true) {
  ...
  if (error) throw new Error(`Création du lien Auth impossible : ${error.message}`);

  const tokenHash = data.properties?.hashed_token;
  if (!tokenHash) throw new Error("Création du lien Auth impossible : token de vérification absent.");
