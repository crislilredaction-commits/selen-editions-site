...
export async function GET() {
  try {
    ...
    if (!access.isManager && !access.trainerProfileId) {
      return NextResponse.json({ error: "Accès réservé au responsable de l'organisme ou à un formateur." }, { status: 403 });
    }
    ...
    const visibleFormations = ((formations ?? []) as FormationRow[]).filter((formation) => {
      if (access.isManager) return true;
      return access.trainerProfileId
        ? jsonArray(formation.allowed_trainer_ids).includes(access.trainerProfileId)
        : false;
    });
    const formationIds = visibleFormations.map((formation) => formation.id);
    if (formationIds.length === 0) {
      return NextResponse.json({ requests: [], actor_types: access.isManager ? ["organisation"] : ["trainer"] });
    }

    const { data: requests, error: requestError } = await access.admin
      .from("daily_formation_registration_requests")
      .select("id,formation_id,response_type,respondent_first_name,respondent_last_name,respondent_email,company_name,participants,adaptation_needed,submitted_at,attached_session_id,status,decision_status,accepted_at,agent_review_requested_at")
    ...
    const { data: decisions, error: decisionError } = requestIds.length
      ? await access.admin
          .from("daily_registration_request_decisions")
          .select("id,registration_request_id,actor_type,decision,comment,decided_at,trainer_profile_id")
...
export async function POST(req: Request) {
  try {
    ...
    if (!requestId || !decision || !requestedActorType) {
      return NextResponse.json({ error: "Décision incomplète." }, { status: 400 });
    }
    if (requestedActorType === "organisation" && !access.isManager) {
      return NextResponse.json({ error: "Seul un responsable de l'organisme peut répondre au nom de l'OF." }, { status: 403 });
    }
    if (requestedActorType === "trainer" && !access.trainerProfileId) {
      return NextResponse.json({ error: "Profil formateur actif requis." }, { status: 403 });
    }
    ...
    if (error) {
      const known = error.message.includes("already accepted")
        ? "Cette candidature a déjà été acceptée."
        : error.message.includes("awaiting agent review")
          ? "Cette candidature a déjà été transmise à Selen pour décision."
          : error.message.includes("permission required")
            ? "Vous n'êtes pas autorisé à statuer sur cette candidature."
            : error.message;
