...
export async function POST(req:Request){
  ...
  const form=await req.formData().catch(()=>null);const file=form?.get("file");const title=String(form?.get("title")??"").trim();const scope=String(form?.get("scope")??"").trim();const sessionId=String(form?.get("session_id")??"").trim()||null;const resourceKind=String(form?.get("resource_kind")??"file");const resourceUrl=cleanUrl(form?.get("url"));let learnerIds:string[]=[];try{learnerIds=JSON.parse(String(form?.get("learner_ids")??"[]")) as string[]}catch{return NextResponse.json({error:"Sélection d'apprenants invalide."},{status:400})}
  if(!title)return NextResponse.json({error:"Indiquez un titre pour la ressource."},{status:400});if(!["organisation","session","learners"].includes(scope))return NextResponse.json({error:"Portée de diffusion invalide."},{status:400});if(scope!=="organisation"&&!sessionId)return NextResponse.json({error:"Sélectionnez une session."},{status:400});if(scope==="learners"&&!learnerIds.length)return NextResponse.json({error:"Sélectionnez au moins un apprenant."},{status:400});
  ...
  let bytes: Buffer; let mimeType: string; let originalName: string;
  if(resourceKind === "link") {
    if(!resourceUrl)return NextResponse.json({error:"Indiquez un lien http ou https valide."},{status:400});
    const escapedTitle=title.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");const escapedUrl=resourceUrl.replaceAll("&","&amp;").replaceAll('"',"&quot;");
    bytes=Buffer.from(`<!doctype html><meta charset="utf-8"><title>${escapedTitle}</title><body style="font-family:Arial,sans-serif;padding:32px"><h1>${escapedTitle}</h1><p><a href="${escapedUrl}" target="_blank" rel="noreferrer">Ouvrir la ressource</a></p><p>${escapedUrl}</p></body>`,`utf8`);mimeType="text/html";originalName=`${safe(title)}.html`;
  } else {
    if(!(file instanceof File)||file.size<=0)return NextResponse.json({error:"Sélectionnez un document."},{status:400});if(file.size>MAX_SIZE)return NextResponse.json({error:"Le document doit peser moins de 10 Mo."},{status:400});if(!ALLOWED.has(file.type))return NextResponse.json({error:"Formats acceptés : PDF, Word, Excel/CSV, JPG, PNG ou WebP."},{status:400});bytes=Buffer.from(await file.arrayBuffer());mimeType=file.type;originalName=file.name;
  }
  const sha256=createHash("sha256").update(bytes).digest("hex");const storagePath=`daily/${context.organisationId}/shared/${Date.now()}-${safe(originalName)}`;const{error:uploadError}=await context.admin.storage.from("documents").upload(storagePath,bytes,{contentType:mimeType,upsert:false});if(uploadError)return NextResponse.json({error:uploadError.message},{status:500});
  const metadata={distribution_scope:scope,session_id:sessionId,learner_ids:scope==="learners"?learnerIds:[],source:"organisation",resource_kind:resourceKind,resource_url:resourceKind==="link"?resourceUrl:null};const{data,error}=await context.admin.from("daily_documents").insert({organisation_id:context.organisationId,document_type:"organisation_shared",linked_object_type:scope,linked_object_id:scope==="organisation"?context.organisationId:sessionId,version:1,status:"published",logical_name:title,bucket:"documents",storage_path:storagePath,mime_type:mimeType,size_bytes:bytes.length,sha256,created_by:context.user.id,updated_by:context.user.id,published_at:new Date().toISOString(),is_current:true,metadata}).select("id,logical_name,status,created_at,metadata").single();if(error){await context.admin.storage.from("documents").remove([storagePath]);return NextResponse.json({error:error.message},{status:500})}return NextResponse.json({document:data},{status:201});
}
