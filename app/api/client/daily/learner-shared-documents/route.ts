...
export async function GET(req:Request){
 ...
 const[docsR,sessionsR,enrolmentsR]=await Promise.all([
  ...
  context.admin.from("daily_session_enrolments").select("id,session_id,learner_id,status,daily_learners(id,first_name,last_name,email)").eq("organisation_id",context.organisationId).not("status","in","(cancelled,declined)"),
 ]);
...
export async function POST(req:Request){
 ...
 if(scope==="learners"){const{data:rows,error}=await context.admin.from("daily_session_enrolments").select("learner_id").eq("organisation_id",context.organisationId).eq("session_id",sessionId!).in("learner_id",learnerIds).not("status","in","(cancelled,declined)");if(error)return NextResponse.json({error:error.message},{status:500});const valid=new Set((rows??[]).map(r=>r.learner_id));if(learnerIds.some(id=>!valid.has(id)))return NextResponse.json({error:"Un apprenant sélectionné n'appartient pas à cette session."},{status:400})}
