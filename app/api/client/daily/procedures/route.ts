...
async function ensureProcedures(organisationId: string) {
  ...
  for (const item of definitions) {
    ...
    const {data:existing}=await admin.from("daily_documents").select("id,metadata").eq("document_type","selen_procedure_model").eq("logical_name",item.procedure_type).eq("is_current",true).order("version",{ascending:false}).limit(1);
