...
export async function GET(request: Request) {
  ...
  if (!sessionId) {
    const { data, error } = await context.admin
      .from("daily_sessions")
      .select("id,internal_reference,start_date,end_date,status,trainer_ids,daily_formations(id,title)")
  ...
  const [{ data: entries, error: entriesError }, { data: enrolments, error: enrolmentsError }] = await Promise.all([
    context.admin
      .from("daily_session_followup_entries")
      .select("id,session_id,enrolment_id,entry_type,level,occurred_at,summary,description,action_taken,status,resolved_at,created_by,created_at,updated_at")
      .eq("organisation_id", context.organisationId)
      .eq("session_id", sessionId)
  ...
  if (entriesError || enrolmentsError) {
    return NextResponse.json({ error: entriesError?.message ?? enrolmentsError?.message }, { status: 500 });
  }
  return NextResponse.json({ session, entries: entries ?? [], enrolments: enrolments ?? [] });
}
  ...
  if (action === "create") {
    ...
    const { data, error } = await context.admin
      ...
      .insert({
        ...
        status: isNote ? "resolved" : "open",
        created_by: context.user.id,
        resolved_by: isNote ? context.user.id : null,
        resolved_at: isNote ? now : null,
      })
      .select("*")
      .single();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
