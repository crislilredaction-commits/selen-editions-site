...
export async function GET(req:Request) {
  ...
  const { data:document,error } = await context.admin.from("daily_documents").select("id,organisation_id,document_type,bucket,storage_path,logical_name,version").eq("id",id).eq("organisation_id",context.organisationId).in("document_type",["training_program","training_agreement","convocation","registration_positioning"]).single();
