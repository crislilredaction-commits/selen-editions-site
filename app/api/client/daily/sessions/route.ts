...
import { sendEnterprisePortalAccessForSessionCompanies } from "@/lib/server/dailyEnterprisePortalAccess";
...
function nullableText(body: Record<string, unknown>, key: string) { const value = text(body, key); return value || null; }
function nullableDate(body: Record<string, unknown>, key: string) { const value = text(body, key); return /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : null; }
...
function cleanSchedule(value: unknown, startDate: string, endDate: string) {
  const blocks = jsonArray(value).map((raw) => {
    ...
    return { date: text(row, "date"), start: text(row, "start"), end: text(row, "end"), note: text(row, "note") };
  ...
  for (const block of blocks) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(block.date) || !/^\d{2}:\d{2}$/.test(block.start) || !/^\d{2}:\d{2}$/.test(block.end)) return { error: "Chaque bloc horaire doit comporter une date, une heure de début et une heure de fin." };
    if (block.end <= block.start) return { error: "L'heure de fin d'un bloc doit être postérieure à l'heure de début." };
    if (block.date < startDate || block.date > endDate) return { error: "Chaque bloc horaire doit être compris entre la date de début et la date de fin de la session." };
  }
  return { blocks };
}
function buildPayload(body: Record<string, unknown>, userId: string, organisationId: string) {
  ...
  if (modality === "distanciel" && !DISTANCE_MODES.has(distanceMode)) return { error: "Précisez si la session à distance est en direct ou à son rythme." };
  ...
  if (maxParticipantsRaw !== null && maxParticipantsRaw !== undefined && String(maxParticipantsRaw).trim() !== "" && maxParticipants === null) return { error: "La capacité maximale doit être un nombre entier supérieur à zéro." };
  ...
  const payload = {
    ...
    location_address: modality === "presentiel" || modality === "mixte" ? nullableText(body, "location_address") : null,
  ...
  if ((modality === "presentiel" || modality === "mixte") && !payload.location_address) return { error: "Renseignez l'adresse physique de la session." };
  if ((modality === "distanciel" || modality === "mixte") && !payload.remote_url) return { error: "Renseignez le lien visio ou la plateforme." };
  return { payload };
}
async function validateTrainerIds(organisationId: string, trainerIds: string[], admin: ReturnType<typeof import("@/lib/server/clientNdaAccess").getAdminSupabase>) {
...
async function refreshLearnerTier(organisationId: string, actingUserId: string, admin: ReturnType<typeof import("@/lib/server/clientNdaAccess").getAdminSupabase>) {
  ...
  const { data: learnerCount, error } = await admin.rpc("daily_prepare_upper_tier_if_needed", { p_user_id: billingUserId });
  if (error) return null;
  return learnerCount ?? null;
}
async function provisionManualCompanyAccess(context: Awaited<ReturnType<typeof getDailyOrganisationContext>>, sessionId: string, req: Request) {
  if (!context.ok) return [];
  try {
    return await sendEnterprisePortalAccessForSessionCompanies(context.admin, { sessionId, origin: new URL(req.url).origin, createdBy: context.user.id, source: "manual_company" });
  } catch (cause) {
    console.error("Daily : session enregistrée mais accès entreprise non finalisé", cause);
    return [];
  }
}
...
export async function POST(req: Request) {
  ...
  if (text(body, "action") === "duplicate") {
    ...
    const { data: source, error: sourceError } = await context.admin.from("daily_sessions").select("*").eq("id", sourceId).eq("organisation_id", context.organisationId).maybeSingle();
  ...
  const { data: formation, error: formationError } = await context.admin.from("daily_formations").select("id,status").eq("id", built.payload.formation_id).eq("organisation_id", context.organisationId).neq("status", "archived").maybeSingle();
...
export async function DELETE(req: Request) {
  ...
  const dependencyChecks = [
    ...
    ["daily_quality_actions", "session_id", "des actions qualité ou d’amélioration"],
  ] as const;
