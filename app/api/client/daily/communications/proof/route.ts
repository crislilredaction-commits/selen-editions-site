...
export async function GET(req: Request) {
  ...
  if (!communicationId) {
    return NextResponse.json({ error: "communication_id requis" }, { status: 400 });
  }

  const { data: communication, error } = await context.admin
  ...
  if (!communication) return NextResponse.json({ error: "Communication introuvable." }, { status: 404 });
  ...
  pdf.setFontSize(9);
  pdf.text("Document généré à partir du registre de communications Selen Daily.", left, y);
  y += 10;
