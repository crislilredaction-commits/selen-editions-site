...
const KINDS = new Set(["positioning", "learning_assessment"]);
...
export async function POST(request: Request) {
  ...
  if (!sessionId || !enrolmentId || !KINDS.has(kind) || !(file instanceof File)) {
    return NextResponse.json({ error: "Session, apprenant, type de preuve et fichier sont requis." }, { status: 400 });
  }
  ...
  if (!enrolment || ["declined", "cancelled"].includes(enrolment.status)) return NextResponse.json({ error: "Inscription introuvable ou inactive." }, { status: 404 });

  const buffer = await file.arrayBuffer();
  const hash = await sha256(buffer);
  const stamp = new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-");
  const originalName = safeName(file.name);
  const documentType = kind === "positioning" ? "positioning_evidence" : "learning_assessment_evidence";
  const folder = kind === "positioning" ? "positioning-evidence" : "evaluation-evidence";
  const storagePath = `daily/${context.organisationId}/session/${sessionId}/${folder}/${enrolmentId}/${stamp}-${originalName}`;
  ...
  const { data: document, error: documentError } = await context.admin
    ...
    .insert({
      ...
      version: 1,
      status: "to_check",
      logical_name: `${kind === "positioning" ? "positionnement" : "evaluation-acquis"}-${originalName}`,
      bucket: "documents",
      ...
      metadata: {
        ...
        source: kind === "positioning" ? "daily_external_positioning" : "daily_external_learning_assessment",
        uploaded_at: new Date().toISOString(),
      },
    })
    .select("id,document_type,logical_name,status,formation_id,session_id,learner_id,enrolment_id,created_at")
    .single();

  if (documentError || !document) {
    await context.admin.storage.from("documents").remove([storagePath]);
    return NextResponse.json({ error: documentError?.message ?? "Enregistrement de la preuve impossible." }, { status: 500 });
  }

  if (kind === "positioning") {
    await context.admin
      .from("daily_session_enrolments")
      .update({ positioning_status: "completed" })
      .eq("id", enrolmentId)
      .eq("organisation_id", context.organisationId);
  }
