...
export async function GET(_request:Request,{params}:Params){
 ...
 const{data:session,error:sessionError}=await admin.from("daily_sessions").select(`id,organisation_id,user_id,daily_formations(${fields})`).eq("id",access.session_id).neq("status","archived").maybeSingle();
 ...
 const{data:org}=await admin.from("daily_onboarding").select("organisation_name").eq("user_id",session.user_id).maybeSingle();
