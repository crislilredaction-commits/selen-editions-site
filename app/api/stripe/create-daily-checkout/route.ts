...
const DAILY_BASE_AMOUNT_CENTS = 6900;
...
export async function POST(request: Request) {
  try {
    ...
    // Le prix de base est défini ici pour éviter qu'un ancien Price ID Stripe
    // configuré à 89 € continue d'être facturé après le changement tarifaire.
    const lineItem = {
      quantity: 1,
      price_data: {
        currency: "eur",
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        pricing_rule_version: "daily_150_2026_08_69",
