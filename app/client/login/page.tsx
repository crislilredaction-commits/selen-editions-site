...
export default function ClientLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (mode === "agent") {
      ...
      if (!agentData) {
        setMessage("Ce compte est connecté, mais il n’a pas d’accès agent actif.");
        setLoading(false);
        return;
      }
    ...
    <main className="gazette-paper min-h-screen text-[#3e2a1f]">
      ...
      <section className="mx-auto max-w-5xl px-4 md:px-6 py-12 md:py-16">
        ...
        <section style={{ marginTop: "1.2rem", background: "var(--paper)", border: "1px solid var(--sepia-mid)", padding: "1.4rem" }}>
          <p className="gazette-label">{mode === "agent" ? "Connexion agent" : "Connexion client"}</p>
          <h2 style={{ color: "var(--ink)", marginBottom: "0.7rem" }}>{mode === "agent" ? "Accéder à l’espace agent" : "Accéder au bureau Selen"}</h2>
          ...
          {message && <div style={{ marginTop: "1rem", border: "1px solid var(--rust)", borderLeft: "4px solid var(--rust)", background: "rgba(138,75,36,0.06)", padding: "0.9rem", color: "var(--rust)", lineHeight: 1.5 }}>{message}</div>}
