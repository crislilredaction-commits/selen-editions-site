...
type Formation = {
  ...
  positioning_questions: Question[];
...
type Workspace = {
  ...
  capabilities?: { trainings?: boolean };
};
...
export default function DailyFormationsPage() {
  ...
  const visibleFormations = useMemo(
    () => formations.filter((formation) => showArchived || formation.status !== "archived"),
    [formations, showArchived],
  );

  function resetForm() {
    setEditingId(null);
    setForm({ ...emptyForm, learning_objectives: [""], positioning_questions: [] });
    setError("");
    setMessage("");
  }
  ...
  function editFormation(formation: Formation) {
    ...
    setForm({
      ...
      registration_methods: formation.registration_methods,
      ...
      positioning_mode: formation.positioning_mode,
      positioning_questions: formation.positioning_questions ?? [],
      results_pending: formation.results_pending,
  ...
  async function archive(id: string) {
    ...
    setMessage(data.archived ? "Formation archivée." : "Formation supprimée.");
    await load();
  }
  ...
  function updateObjective(index: number, value: string) {
    setForm((current) => ({
      ...current,
      learning_objectives: current.learning_objectives.map((objective, i) => i === index ? value : objective),
    }));
  }
  ...
  function updateQuestion(index: number, patch: Partial<Question>) {
    setForm((current) => ({
      ...current,
      positioning_questions: current.positioning_questions.map((question, i) => i === index ? { ...question, ...patch, order: i + 1 } : question),
    }));
  }

  if (loading) return <main style={styles.main}><p>Chargement du catalogue…</p></main>;
  if (workspace && workspace.capabilities?.trainings === false) {
    return <main style={styles.main}><h1>Formations</h1><p>Ton accès ne comprend pas la gestion des formations.</p></main>;
  }
  ...
  return (
    <main style={styles.main}>
      <header style={styles.hero}>
        <div>
          <p style={styles.eyebrow}>Selen Daily · Catalogue</p>
          <h1 style={styles.h1}>Formations</h1>
          <p style={styles.lead}>Une formation est un modèle réutilisable. Les dates, participants et formateurs opérationnels vivent ensuite dans les sessions.</p>
        </div>
        <div style={styles.stat}><strong>{formations.filter((f) => f.status !== "archived").length}</strong><span>formations actives</span></div>
      </header>
      ...
      <section style={styles.card}>
        <div style={styles.sectionTitle}>
          <div><h2 style={styles.h2}>{editingId ? "Modifier la formation" : "Nouvelle formation"}</h2><p style={styles.muted}>Les champs structurants alimenteront les programmes et futurs documents Daily.</p></div>
        ...
        <form onSubmit={save} style={styles.form}>
          ...
          <div style={styles.full}>
            <label style={styles.label}>Objectifs pédagogiques *</label>
            {form.learning_objectives.map((objective, index) => (
              <div key={index} style={styles.row}>
                <input value={objective} onChange={(e) => updateObjective(index, e.target.value)} placeholder={`Objectif ${index + 1}`} style={{ ...styles.input, flex: 1 }} />
                {form.learning_objectives.length > 1 ? <button type="button" style={styles.smallButton} onClick={() => setForm({ ...form, learning_objectives: form.learning_objectives.filter((_, i) => i !== index) })}>Retirer</button> : null}
              </div>
            ))}
            <button type="button" style={styles.secondary} onClick={() => setForm({ ...form, learning_objectives: [...form.learning_objectives, ""] })}>+ Ajouter un objectif</button>
          </div>
          ...
          {form.positioning_mode === "selen" ? (
            <div style={styles.full}>
              <label style={styles.label}>Questions de positionnement</label>
              {form.positioning_questions.map((question, index) => (
                <div key={question.id} style={styles.questionCard}>
                  <div style={styles.grid2}>
                    <Field label={`Question ${index + 1}`}><input value={question.label} onChange={(e) => updateQuestion(index, { label: e.target.value })} style={styles.input} /></Field>
                    <Field label="Type"><select value={question.type} onChange={(e) => updateQuestion(index, { type: e.target.value as Question["type"], options: ["single_choice", "multiple_choice"].includes(e.target.value) ? question.options : [] })} style={styles.input}><option value="free_text">Texte libre</option><option value="single_choice">Choix unique</option><option value="multiple_choice">Choix multiple</option><option value="scale_1_5">Échelle 1 à 5</option></select></Field>
                  </div>
                  {["single_choice", "multiple_choice"].includes(question.type) ? <Field label="Options (une par ligne)"><textarea value={question.options.join("\n")} onChange={(e) => updateQuestion(index, { options: e.target.value.split("\n").map((v) => v.trim()).filter(Boolean) })} style={styles.textarea} /></Field> : null}
                  <label style={styles.checkbox}><input type="checkbox" checked={question.required} onChange={(e) => updateQuestion(index, { required: e.target.checked })} /> Réponse obligatoire</label>
                  <button type="button" style={styles.smallButton} onClick={() => setForm({ ...form, positioning_questions: form.positioning_questions.filter((_, i) => i !== index) })}>Retirer la question</button>
                </div>
              ))}
              <button type="button" style={styles.secondary} onClick={() => setForm({ ...form, positioning_questions: [...form.positioning_questions, newQuestion(form.positioning_questions.length)] })}>+ Ajouter une question</button>
            </div>
          ) : null}
      ...
      <section style={styles.card}>
        <div style={styles.sectionTitle}>
          <div><h2 style={styles.h2}>Catalogue</h2><p style={styles.muted}>Duplique une formation récurrente au lieu de tout ressaisir.</p></div>
            ...
            {visibleFormations.map((formation) => (
              <article key={formation.id} style={styles.listCard}>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={styles.muted}>{formation.duration_hours} h · {formation.duration_days} j · {formation.modality}</p>
                ...
                <div style={styles.actions}>
                  {formation.status !== "archived" ? <button type="button" style={styles.secondary} onClick={() => editFormation(formation)}>Modifier</button> : null}
                  <button type="button" style={styles.secondary} onClick={() => void duplicate(formation.id)}>Dupliquer</button>
                  {formation.status !== "archived" ? <button type="button" style={styles.smallButton} onClick={() => void archive(formation.id)}>Archiver</button> : null}
                </div>
              </article>
            ))}
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  form: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: "1rem" },
  ...
  largeTextarea: { width: "100%", minHeight: 150, padding: ".72rem .78rem", border: "1px solid var(--sepia-mid)", background: "#fffdf8", color: "var(--ink)", boxSizing: "border-box", resize: "vertical" },
  ...
  grid2: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: ".8rem" },
  questionCard: { border: "1px solid var(--sepia-mid)", padding: ".8rem", display: "grid", gap: ".7rem", background: "rgba(201,160,85,.04)" },
