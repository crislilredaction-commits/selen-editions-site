...
export default function DailyDossiersPage() {
  ...
  return <main style={{maxWidth:1100,margin:"0 auto",padding:"2rem 1rem 4rem"}}>
    ...
    {sessions.length===0?<section style={card}>Aucun dossier de session pour le moment.</section>:<>
      ...
      {session?<section style={{...card,marginBottom:20}}><strong>{formationMap.get(session.formation_id)||"Formation"}</strong><div style={{marginTop:8}}>Progression : {done}/{sessionItems.length} · {progress}%</div><div style={{height:8,background:"rgba(120,90,60,.15)",marginTop:8}}><div style={{height:"100%",width:`${progress}%`,background:"var(--rust)"}}/></div><div style={{marginTop:14,paddingTop:12,borderTop:"1px solid var(--sepia-mid)"}}><strong>Preuves d’audit de cette session</strong><p style={{margin:"6px 0 10px",fontSize:14}}>Retrouvez les e-mails tracés, leurs pièces jointes figées et les preuves PDF d’envoi déjà enregistrées pour ce dossier.</p><a href={`/client/daily/communications?session_id=${encodeURIComponent(session.id)}`} style={{display:"inline-block",border:"1px solid var(--rust)",padding:".6rem .8rem",color:"var(--rust)",fontWeight:700,textDecoration:"none"}}>Voir les communications et preuves</a></div></section>:null}
