...
export default function DailyRegistrationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const [participants, setParticipants] = useState<Participant[]>([{ first_name: "", last_name: "", email: "" }]);
    ...
    <main className="gazette-paper min-h-screen">
      <section style={s.page}>
        ...
        <article style={s.hero}>
          <p className="gazette-label">Dossier d&apos;inscription</p>
          <h1 style={s.title}>Préparons votre entrée en formation</h1>
          <p style={s.muted}>{session?.daily_formations?.title ?? "Formation"}</p>
...
function Welcome({ mode, setMode }: { mode: RegistrationMode; setMode: (mode: RegistrationMode) => void }) {
  return (
    <div style={s.stack}>
      <h2 style={s.sectionTitle}>Quelques informations pour préparer votre inscription</h2>
      <p style={s.muted}>Ces questions permettent à l&apos;organisme de formation de comprendre votre situation et vos besoins avant l&apos;entrée en formation. Il n&apos;y a pas de bonne ou de mauvaise réponse.</p>
      <div style={s.toggle}>
        <button type="button" className={mode === "beneficiary" ? "btn-ink" : "btn-ghost"} onClick={() => setMode("beneficiary")}><span>Je suis apprenant</span></button>
        <button type="button" className={mode === "company" ? "btn-ink" : "btn-ghost"} onClick={() => setMode("company")}><span>Je représente une entreprise</span></button>
      </div>
    </div>
  );
}

function BeneficiaryStep({ step, form, update, hasSelenPositioning, positioningQuestions }: { step: number; form: Record<string, string>; update: (key: string, value: string) => void; hasSelenPositioning: boolean; positioningQuestions: PositioningQuestion[] }) {
  if (step === 1) return (
    <div style={s.stack}>
      ...
      <div style={s.grid}>
        ...
        <Input label="Téléphone" value={form.phone} onChange={(value) => update("phone", value)} required />
  ...
  if (step === 4) return (
    <div style={s.stack}>
      ...
      <Textarea label="Contraintes personnelles ou d'organisation : temps, matériel, transport, disponibilités..." value={form.constraints} onChange={(value) => update("constraints", value)} />
  ...
  if (step === 6) return (
    <div style={s.stack}>
      ...
      <p style={s.notice}>Ces questions permettent à l&apos;organisme de formation d&apos;évaluer votre niveau de départ.</p>
      {positioningQuestions.map((question) => <PositioningQuestionField key={question.id} question={question} value={form[`positioning_${question.id}`] ?? ""} onChange={(value) => update(`positioning_${question.id}`, value)} />)}
    </div>
  );
  return (
    <div style={s.stack}>
      <h2 style={s.sectionTitle}>Besoins de compensation et contraintes personnelles</h2>
      <p style={s.muted}>Cette question concerne les besoins de compensation liés à une situation de handicap, ainsi que les contraintes personnelles qui pourraient nécessiter une adaptation de la formation.</p>
      <Select label="Avez-vous un besoin ou une contrainte particulière à prendre en compte pour suivre cette formation dans de bonnes conditions ?" value={form.adaptation_needed} onChange={(value) => update("adaptation_needed", value)} options={["non", "oui"]} />
      {form.adaptation_needed === "oui" ? (
        <>
          <Textarea label="Précisez, si vous le souhaitez, les besoins ou contraintes dont vous souhaitez nous informer" value={form.adaptation_details} onChange={(value) => update("adaptation_details", value)} />
          <p style={s.callout}>Vous serez recontacté(e) par téléphone par l&apos;organisme de formation afin d&apos;échanger avec vous sur vos besoins et de rechercher avec vous les adaptations appropriées.</p>
        </>
      ) : null}
      {!hasSelenPositioning ? <p style={s.notice}>Le positionnement sera complété selon les modalités prévues par l&apos;organisme de formation.</p> : null}
    </div>
  );
}

function CompanyStep({ step, form, update, participants, setParticipants, updateParticipant }: { step: number; form: Record<string, string>; update: (key: string, value: string) => void; participants: Participant[]; setParticipants: (participants: Participant[]) => void; updateParticipant: (index: number, patch: Partial<Participant>) => void }) {
  if (step === 1) return <div style={s.stack}><h2 style={s.sectionTitle}>Entreprise</h2><Input label="Nom de l'entreprise" value={form.company_name} onChange={(value) => update("company_name", value)} /><Input label="SIRET" value={form.company_siret} onChange={(value) => update("company_siret", value)} /><Textarea label="Adresse" value={form.company_address} onChange={(value) => update("company_address", value)} /></div>;
  if (step === 2) return (
    <div style={s.stack}><h2 style={s.sectionTitle}>Contacts</h2><div style={s.grid}>
      <Input label="Contact administratif - nom" value={form.admin_contact_name} onChange={(value) => update("admin_contact_name", value)} /><Input label="Fonction" value={form.admin_contact_role} onChange={(value) => update("admin_contact_role", value)} /><Input label="Email" type="email" value={form.admin_contact_email} onChange={(value) => update("admin_contact_email", value)} /><Input label="Téléphone" value={form.admin_contact_phone} onChange={(value) => update("admin_contact_phone", value)} />
      <Input label="Contact formation - nom" value={form.training_contact_name} onChange={(value) => update("training_contact_name", value)} /><Input label="Fonction" value={form.training_contact_role} onChange={(value) => update("training_contact_role", value)} /><Input label="Email" type="email" value={form.training_contact_email} onChange={(value) => update("training_contact_email", value)} /><Input label="Téléphone" value={form.training_contact_phone} onChange={(value) => update("training_contact_phone", value)} />
    </div></div>
  );
  if (step === 3) return <FundingFields form={form} update={update} allowed={["entreprise", "opco", "cpf", "autre"]} />;
  if (step === 4) return (
    <div style={s.stack}><h2 style={s.sectionTitle}>Besoin entreprise</h2><Textarea label="Besoin exprimé par l'entreprise" value={form.expressed_need} onChange={(value) => update("expressed_need", value)} /><Textarea label="Objectifs attendus pour les salariés" value={form.employee_objectives} onChange={(value) => update("employee_objectives", value)} /><Textarea label="Contexte de la demande" value={form.request_context} onChange={(value) => update("request_context", value)} /><Textarea label="Contraintes organisationnelles" value={form.constraints} onChange={(value) => update("constraints", value)} /><Select label="Modalités souhaitées" value={form.preferred_modality} onChange={(value) => update("preferred_modality", value)} options={modalities} /><Textarea label="Précisions" value={form.details} onChange={(value) => update("details", value)} /><Textarea label="Demandes ou points d'attention particuliers" value={form.specific_requests} onChange={(value) => update("specific_requests", value)} /></div>
  );
  if (step === 5) return <div style={s.stack}><h2 style={s.sectionTitle}>Participants</h2><ParticipantRows rows={participants} setRows={setParticipants} updateParticipant={updateParticipant} /></div>;
  return (
    <div style={s.stack}>
      <h2 style={s.sectionTitle}>Besoins particuliers connus</h2>
      <p style={s.muted}>Indiquez seulement si vous savez qu&apos;un participant a un besoin de compensation lié à une situation de handicap ou une contrainte personnelle pouvant nécessiter une adaptation. Les détails seront échangés directement avec la personne concernée.</p>
      <Select label="Un besoin particulier est-il connu pour un ou plusieurs participants ?" value={form.company_adaptation_needed} onChange={(value) => update("company_adaptation_needed", value)} options={["non", "oui"]} />
      {form.company_adaptation_needed === "oui" ? (
        <>
          <Textarea label="Précision générale, sans information médicale confidentielle" value={form.company_adaptation_details} onChange={(value) => update("company_adaptation_details", value)} />
          <p style={s.callout}>La personne concernée sera recontactée par téléphone par l&apos;organisme de formation afin d&apos;échanger directement sur ses besoins et les adaptations possibles.</p>
        </>
      ) : null}
      <p style={s.notice}>Chaque participant pourra ensuite préciser sa propre situation dans son dossier individuel.</p>
    </div>
  );
}

function PositioningQuestionField({ question, value, onChange }: { question: PositioningQuestion; value: string; onChange: (value: string) => void }) {
  const options = Array.isArray(question.options) ? question.options : [];
  const selectedValues = value ? value.split("|||") : [];
  function toggleOption(option: string) {
    onChange((selectedValues.includes(option) ? selectedValues.filter((item) => item !== option) : [...selectedValues, option]).join("|||"));
  }
  return (
    <fieldset style={s.questionBox}>
      ...
      {question.type === "single_choice" ? <div style={s.choiceGrid}>{options.map((option) => <label key={option} style={s.choice}><input type="radio" checked={value === option} onChange={() => onChange(option)} />{option}</label>)}</div> : null}
      {question.type === "multiple_choice" ? <div style={s.choiceGrid}>{options.map((option) => <label key={option} style={s.choice}><input type="checkbox" checked={selectedValues.includes(option)} onChange={() => toggleOption(option)} />{option}</label>)}</div> : null}
    </fieldset>
  );
}
...
function FundingFields({ form, update, allowed }: { form: Record<string, string>; update: (key: string, value: string) => void; allowed: readonly string[] }) {
  return <div style={s.stack}><h2 style={s.sectionTitle}>Financement</h2><Select label="Mode de financement envisagé" value={form.funding} onChange={(value) => update("funding", value)} options={allowed} />{form.funding === "autre" ? <Input label="Précisez le financement" value={form.funding_other} onChange={(value) => update("funding_other", value)} /> : null}</div>;
}
...
function ParticipantRows({ rows, setRows, updateParticipant }: { rows: Participant[]; setRows: (rows: Participant[]) => void; updateParticipant: (index: number, patch: Partial<Participant>) => void }) {
  return <div style={s.subcard}><strong>Liste des participants à vérifier ou compléter</strong>{rows.map((row, index) => <div key={index} style={s.grid}><Input label="Prénom" value={row.first_name} onChange={(value) => updateParticipant(index, { first_name: value })} /><Input label="Nom" value={row.last_name} onChange={(value) => updateParticipant(index, { last_name: value })} /><Input label="Email" type="email" value={row.email} onChange={(value) => updateParticipant(index, { email: value })} /></div>)}<button type="button" className="btn-ghost" onClick={() => setRows([...rows, { first_name: "", last_name: "", email: "" }])}><span>Ajouter un participant</span></button></div>;
}

function Input({ label, value, onChange, type = "text", required = false }: { label: string; value?: string; onChange: (value: string) => void; type?: string; required?: boolean }) {
  return <label style={s.field}><span>{label}</span><input style={s.input} type={type} value={value ?? ""} required={required} onChange={(event) => onChange(event.target.value)} /></label>;
}
function Textarea({ label, value, onChange }: { label: string; value?: string; onChange: (value: string) => void }) {
  return <label style={s.field}><span>{label}</span><textarea style={{ ...s.input, minHeight: 96 }} value={value ?? ""} onChange={(event) => onChange(event.target.value)} /></label>;
}
function Select({ label, value, onChange, options }: { label: string; value?: string; onChange: (value: string) => void; options: readonly string[] }) {
  return <label style={s.field}><span>{label}</span><select style={s.input} value={value ?? ""} onChange={(event) => onChange(event.target.value)}><option value="">Sélectionner</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
}
...
const s: Record<string, React.CSSProperties> = {
  ...
  stack: { display: "grid", gap: "0.9rem" }, grid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.7rem" },
  ...
  actions: { display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.7rem" }, toggle: { display: "flex", flexWrap: "wrap", gap: "0.6rem" },
  subcard: { display: "grid", gap: "0.7rem", border: "1px solid rgba(178,138,98,0.28)", padding: "0.8rem" },
  questionBox: { display: "grid", gap: "0.65rem", border: "1px solid rgba(178,138,98,0.35)", padding: "0.85rem" }, questionLegend: { color: "var(--ink)", fontWeight: 800, padding: "0 0.3rem" },
  choiceGrid: { display: "flex", flexWrap: "wrap", gap: "0.55rem" }, choice: { display: "inline-flex", gap: "0.4rem", alignItems: "center", border: "1px solid rgba(178,138,98,0.35)", padding: "0.55rem 0.7rem", color: "var(--ink)" },
  powered: { textAlign: "center", color: "var(--ink-soft)", fontSize: "0.8rem", margin: "0.25rem 0 0" },
};
