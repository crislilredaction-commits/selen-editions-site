...
import LearnerPhaseNavigation from "@/components/daily/LearnerPhaseNavigation";
...
export default async function DailyPortalPage({ params }: { params: Promise<{ role: string; token: string }> }) {
  ...
  const learner = ["learner", "apprenant"].includes(role);
  return <>
    {learner ? <LearnerPhaseNavigation role={role} token={token} /> : null}
