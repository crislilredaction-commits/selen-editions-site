...
export default function AgentAuditBlancDetailPage() {
  ...
  async function loadDetail() {
    ...
    const { data: reportData, error: reportError } = await supabase
      ...
      .select(
        "id, case_id, global_summary, strengths, major_nonconformities, minor_nonconformities, corrective_actions, final_recommendation, is_visible_to_client, created_by_email, created_at, updated_at",
      )
      .eq("case_id", caseId)
      .maybeSingle();
  ...
  function updateReportField(
    ...
    setReport((prev) => {
      ...
      return {
        ...
        created_by_email: agent?.email ?? null,
  ...
  async function saveReport(visibleToClient?: boolean) {
    if (!auditCase || !agent) {
      setError("Dossier ou agent introuvable.");
      return;
    }
    ...
    const reportPayload = {
      ...
      major_nonconformities: report?.major_nonconformities ?? "",
      minor_nonconformities: report?.minor_nonconformities ?? "",
      ...
      is_visible_to_client:
        typeof visibleToClient === "boolean"
          ? visibleToClient
          : (report?.is_visible_to_client ?? false),
    ...
    if (reportError) {
      setError(`Impossible de sauvegarder le rapport : ${reportError.message}`);
      setSavingReport(false);
      return;
    }

    const nextReportStatus = reportPayload.is_visible_to_client
      ? "sent"
      : "draft";
    ...
    const { error: caseUpdateError } = await supabase
      ...
      .update({
        ...
        status: reportPayload.is_visible_to_client
          ? "report_ready"
          : auditCase.status,
        updated_at: new Date().toISOString(),
      })
    ...
    if (caseUpdateError) {
      setError(
        `Rapport sauvegardé, mais statut dossier non mis à jour : ${caseUpdateError.message}`,
      );
      setSavingReport(false);
      return;
    }

    setReport(reportData as AuditBlancReport);
    setAuditCase((prev) =>
      prev
        ? {
            ...prev,
    ...
    setSuccess(
      reportPayload.is_visible_to_client
        ? "Rapport sauvegardé et visible dans l’espace client."
          ...
          >
            <section style={{ display: "grid", gap: "1rem" }}>
              ...
              >
                ...
                <h2 style={{ color: "var(--ink)", marginBottom: "0.7rem" }}>
                  Rédiger le rapport d’audit blanc
                </h2>
                ...
                >
                  Ce rapport sera visible côté client uniquement lorsque vous le
                  publierez. Vous pouvez le sauvegarder en brouillon pendant la
                  préparation.
                </p>
                ...
                <div style={{ display: "grid", gap: "0.9rem" }}>
                  {[
                    {
                      ...
                      field: "global_summary",
                      placeholder:
                        "Résumé global de l’audit blanc : niveau de préparation, points principaux, vigilance générale...",
                    },
                    {
                      label: "Points conformes / éléments solides",
                      field: "strengths",
                      placeholder:
                        "Ex : documents déjà structurés, preuves disponibles, organisation claire...",
                    },
                    {
                      label: "Non-conformités majeures probables",
                      field: "major_nonconformities",
                      placeholder:
                        "Listez les risques majeurs identifiés et les indicateurs concernés.",
                    },
                    {
                      label: "Non-conformités mineures / points de vigilance",
                      field: "minor_nonconformities",
                      placeholder:
                        "Listez les écarts mineurs, imprécisions ou preuves à renforcer.",
                    },
                    {
                      label: "Actions correctives recommandées",
                      field: "corrective_actions",
                      placeholder:
                        "Indiquez les actions à mener avant l’audit : documents à créer, preuves à récupérer, procédures à clarifier...",
                    },
                    {
                      label: "Conclusion / recommandation finale",
                      field: "final_recommendation",
                      placeholder:
                        "Ex : prêt pour l’audit, audit à décaler, corrections prioritaires avant passage...",
                    },
                  ].map((item) => (
                    <label
                      key={item.field}
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={report?.is_visible_to_client ?? false}
                      onChange={(event) =>
                        setReport((prev) =>
                          prev
                            ? {
                    ...
                    />
                    Rapport visible dans l’espace client
                  </label>
