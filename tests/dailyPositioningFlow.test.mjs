...
const[migration,api,portal,page,workspace]=await Promise.all([readFile(migrationPath,"utf8"),readFile(apiPath,"utf8"),readFile(portalPath,"utf8"),readFile(pagePath,"utf8"),readFile(workspacePath,"utf8")]);
...
test("l'espace apprenant ouvre directement le positionnement tant qu'il n'est pas terminé",()=>{assert.match(workspace,/positioning_status/);assert.match(workspace,/\/positionnement/);assert.match(workspace,/positioningDone\(positioning\)/)});
test("le formateur voit l'état canonique du positionnement sans recevoir les réponses détaillées",()=>{assert.match(portal,/participantProgress/);assert.match(portal,/positioningStatus:row\.positioning_status/);assert.doesNotMatch(portal,/participantProgress[\s\S]*answers:/);assert.match(workspace,/Positionnement des participants/);assert.match(workspace,/Réalisé/);assert.match(workspace,/À faire/)});
