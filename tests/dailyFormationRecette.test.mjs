...
test("les documents de recette utilisent un import de fichier contrôlé", () => {
  for (const kind of ["organisation_logo", "insee_notice", "qualiopi_certificate", "bpf", "trainer_cv", "training_program_source", "positioning_questionnaire_source"]) assert.match(uploadRoute, new RegExp(kind));
...
test("le NDA est exigé lorsqu'il existe et les imports utilisent un bouton encadré explicite", () => {
  ...
  assert.match(onboardingPage, /style=\{s\.fileInput\}/); assert.match(onboardingPage, /style=\{s\.fileButton\}/); assert.match(onboardingPage, /Choisir un fichier/);
});

test("le client peut demander un accompagnement pendant tout le paramétrage autonome", () => {
  assert.match(onboardingPage, /form\.current_step > 1 && form\.setup_choice === "self"/); assert.match(onboardingPage, /Je souhaite être accompagné/); assert.match(onboardingPage, /setup_choice: "video" as const/);
});

test("les parcours initiaux restent isolés de la navigation courante sans retirer l'assistance", () => {
  assert.match(dailyLayout, /pathname === "\/client\/daily\/onboarding" \|\| pathname === "\/client\/daily\/invitation"/); assert.match(dailyLayout, /!isStandaloneFlow/); assert.match(dailyLayout, /DailyFriendlyBanner/); assert.match(onboardingPage, /<ClientSupportBar/);
});

test("la création d'une formation est suivie par la création de sa session dédiée", () => {
  assert.match(formationsManager, /router\.push\(`\/client\/daily\/sessions\/new\?formation=\$\{encodeURIComponent\(formationId\)\}`\)/); assert.doesNotMatch(formationsManager, /showSessionForm/);
});

test("une formation non validée est modifiée en place sans recréer son token public", () => {
  assert.match(formationsRoute, /\["draft", "review", "correction_requested"\]\.includes\(existing\.status\)/); assert.match(formationsRoute, /\.from\("daily_formations"\)\.update\(\{/); assert.match(formationsRoute, /public_registration_token: existing\.public_registration_token \?\? registrationToken\(\)/); assert.match(formationsRoute, /versioned: false/);
});
...
test("P0-D ne force pas la ressaisie descriptive lors d'un import de programme", () => {
  assert.match(creationPolicy, /PROGRAM_IMPORT_REQUIRED_COMPLEMENTS/); assert.match(creationPolicy, /validateFormationCreationSource/); assert.match(creationPolicy, /requiresStructuredLearningObjectives/);
