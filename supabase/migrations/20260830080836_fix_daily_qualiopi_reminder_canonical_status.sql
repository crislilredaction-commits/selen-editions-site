...
begin
  ...
  if v_org.qualiopi_status = 'certified' and v_email is not null and v_org.qualiopi_surveillance_window_start is not null then
    v_due_at := v_org.qualiopi_surveillance_window_start::timestamp at time zone 'Europe/Paris';
    update public.client_reminders
    set client_email = v_email, due_at = v_due_at,
        ...
        metadata = v_metadata || jsonb_build_object('cycle_stage','surveillance','window_start',v_org.qualiopi_surveillance_window_start,'window_end',v_org.qualiopi_surveillance_window_end),
    ...
    if v_rows = 0 then
      insert into public.client_reminders (
        ...
        v_due_at,
        v_metadata || jsonb_build_object('cycle_stage','surveillance','window_start',v_org.qualiopi_surveillance_window_start,'window_end',v_org.qualiopi_surveillance_window_end),
    ...
    update public.client_reminders set status='ignored'
    where dedupe_key='daily:qualiopi:surveillance:'||v_org.id::text
      and status in ('draft','ready','postponed');
  end if;

  if v_org.qualiopi_status = 'certified' and v_email is not null and v_org.qualiopi_renewal_reminder_on is not null then
    ...
    set client_email=v_email, due_at=v_due_at,
        ...
        metadata=v_metadata || jsonb_build_object('cycle_stage','renewal','valid_until',v_org.qualiopi_valid_until),
    ...
    if v_rows = 0 then
      insert into public.client_reminders (
        ...
        v_due_at,
        v_metadata || jsonb_build_object('cycle_stage','renewal','valid_until',v_org.qualiopi_valid_until),
  ...
  if v_org.qualiopi_status = 'certified' and v_email is not null and v_org.qualiopi_valid_until is not null then
    ...
    set client_email=v_email, due_at=v_due_at,
        ...
        metadata=v_metadata || jsonb_build_object('cycle_stage','certificate_expiry','valid_until',v_org.qualiopi_valid_until),
    ...
    if v_rows = 0 then
      insert into public.client_reminders (
        ...
        v_due_at,
        v_metadata || jsonb_build_object('cycle_stage','certificate_expiry','valid_until',v_org.qualiopi_valid_until),
...
from public.organisations
where qualiopi_status = 'certified';
